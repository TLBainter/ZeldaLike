...
@tool
class_name InteractableComponentContainer
...
func _finish(user : Player) -> void:
	...
	# Resolve dungeon items to a dungeon-specific id, or fall back to an orange bead.
	if _resolved_item_kind == ContainerRewardResource.ItemKind.DUNGEON_ITEM:
		...
		if level == null or level.get_effective_type() != Level.LevelType.DUNGEON:
			...
			_resolved_item_id = level.get_effective_name().to_lower() + "_" + _resolved_item_id
