...
func enter() -> void:
	...
	# Consume magic — mirrors State_Attack's energy cost pattern.
	...
	# Connect damage detection for the interruptible phase.
	if character.health:
		_safe_connect(character.health.health_changed, _on_health_changed)

	# Play the directional cast animation.
...
func _process(_delta : float) -> void:
	...
	if not character or not character.anim:
		_exit_to_no_action()
		return
	# Fallback: if the animation ends without on_end_spell_casting firing, exit cleanly.
...
func on_cast_spell() -> void:
	...
	# TODO: trigger actual spell effect here.
...
func _on_health_changed(_cur : int, _max : int, change_amount : int) -> void:
	# Negative change_amount = damage received.
